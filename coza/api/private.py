...
from coza.errors import CozaRequestException
...
class TradeApi(object):
    ...
    @classmethod
    def order_list(cls, data):
        """
            coinone : {'currency': currency}
            upbit : {
                'market_id':market_id,
                'state':state,
                'page':page,
        ...
        result = _request(url, 'GET', cls.user_uuid, data=data)
        return result

    # bot의 order 취소 api
    @classmethod
    def order_cancel(cls, data):
        """
            coinone : {
    ...
    @classmethod
    def order_status(cls, data):
        """
            coinone : {'currency': currency, 'order_id': order_id}
            upbit : {'order_id': order_id}
        """
    ...
    @classmethod
    def order_chance(cls, data):
        """
            upbit : {'market_id':'KRW-XRP'}
        """
        url = f'{COZA_HOST}/users/{cls.user_uuid}/bots/{cls.bot_id}/orderchance'
        result = _request(url, 'GET', cls.user_uuid, data=data)
    ...
    # bot stop api
    """
        bot safety setting 조건시 bot stop
        data={'message': "ex) profit target or trailing stop message"}
    """
    @classmethod
    def bot_stop(cls, data=None):
        url = f'{COZA_HOST}/users/{cls.user_uuid}/bots/{cls.bot_id}/stop'
        data = _request(url, 'POST', cls.user_uuid, data=data)
        return data
